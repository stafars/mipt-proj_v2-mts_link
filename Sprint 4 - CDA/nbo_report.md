...
### Модель
LightGBM (градиентный бустинг) с многоклассовой классификацией.
...
6. **440 компаний без рекомендаций** — для них недостаточно истории покупок (только одна транзакция). Для таких клиентов можно использовать fallback-стратегию: рекомендовать топ-3 наиболее часто покупаемых продуктов после первой покупки на основе агрегированной статистики по всем компаниям. Это позволит покрыть 100% клиентской базы, хотя качество рекомендаций будет ниже, чем для клиентов с историей.
