...
Этот репозиторий содержит ноутбук анализа клиентских данных и несколько подготовленных датасетов в форматах Excel и CSV.  
Ниже описаны исходные файлы и новые агрегированные таблицы.
...
**Возможные добавленные временные признаки (на основе `sale_date`, если они были сохранены):**
...
ltv = (
    revenue_df
        ...
        .sum()
        .rename(columns={'product_sum': 'ltv'})
)
````
...
Агрегация по клиенту и дополнительные числовые признаки:

* `recency` — количество дней с момента **последней покупки** до `snapshot_date`:
* `customer_age_days` — «возраст» клиента: число дней с **первой покупки** до `snapshot_date`:
