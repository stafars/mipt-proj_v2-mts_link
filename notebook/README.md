...
* `first_purchase` — дата первой покупки клиента.
* `last_purchase` — дата последней покупки клиента.
* `ltv` — суммарная выручка от клиента за весь период наблюдения (nullable `Int64`).
* `customer_age_days` — возраст клиента в днях от первой до последней покупки.
* `customer_age_years` — возраст клиента в годах (конвертировано из дней).
* `ltv_per_year` — среднегодовая ценность клиента (ltv разделённая на customer_age_years).
* `ltv_12m` — выручка от клиента за последние 12 месяцев (nullable `Int64`).

**Использование:**

* анализ ценности клиентов и их сегментация,
